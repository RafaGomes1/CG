...
TranslationDynamic::TranslationDynamic(float time, vector<Point> p, bool isAligned) {
    ...
    while (gt < 1) {
        getGlobalCatmullRomPoint(gt, pos, deriv, points.size(), points);
        vbo_data.push_back(pos[0]);
        vbo_data.push_back(pos[1]);
        vbo_data.push_back(pos[2]);
        gt += 1.0 / 100;
    }
...
void TranslationDynamic::renderCatmullRomCurve() {
    if (vbo == 0) return;
